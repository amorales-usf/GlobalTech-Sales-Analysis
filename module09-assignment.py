...
# group by region, sum total sales for each region, sort in descending order
...
# Your code here
missing_counts = sales_df.isna().sum() # count missing values per column
...
# Your code here
missing_percentages = (missing_counts / len(sales_df)) * 100 # convert to percentages
...
# reshape grouped data into a table and compare categories
top_categories_by_region = sales_df.groupby(["Region", "Category"])["Total_Sales"].sum().unstack().idxmax(axis=1)
...
# Your code here
product_totals = sales_df.groupby("Product")["Total_Sales"].sum() # total revenue per product
product_revenue_analysis = pd.DataFrame({
    "total_revenue": product_totals,
    "percentage": (product_totals / total_revenue) * 100 # percent of total sales
})
...
print(f"- Average Sale Value: ${total_revenue / total_units:.2f}")
...
# Your code here
print("\nRegional Performance:")
for region, value in sales_by_region.items(): # loop through regions
...
# Your code here
print("\nCategory Performance:")
for category in avg_units_by_category.index: #loop through categories
...
print("1. Increase promotions on high-performing products.")
print("2. Focus on top-performing regions.")
print("3. Improve data collection to reduce missing values.")
